export default {
  nav: {
    logo: 're:name',
    links: [
      { text: 'works', to: 'mywork' },
      { text: 'connect', to: 'mycontact' },
    ],
  },
  header: {
    img: 'https://images.unsplash.com/photo-1634949454472-44d448f9be14?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=435&q=80',
    imgPlaceholder:
      'data:image/jpeg;base64,',
    text: ['Hi!', "It's me, Maksim", 'I am '],
    typical: ['web developer. 🖥', 2000, 'photographer. 📹', 2000, 'fashion influencer.', 2000],
    btnText: 'what i use',
  },

  stack: {
    title: 'Stack',
    tech: [
      {
        img: process.env.PUBLIC_URL + '/assets/bootstrap.png',
        alt: 'bootstrap',
      },
      {
        img: process.env.PUBLIC_URL + '/logo512.png',
        alt: 'react',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/django.png',
        alt: 'django',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/redux.png',
        alt: 'redux',
      },
    ],
    desc: `Bootstrap can be used as a styling tool for web applications. 
    React is used as the main front-end technology.
    Django provides the back-end to create full-fledged web applications.
    Redux serves as a supplement to React for better storage states.`,
  },

  works: {
    slider: {
      first: {
        img: process.env.PUBLIC_URL + '/assets/slide1.png',
      },
      second: {
        img: process.env.PUBLIC_URL + '/assets/slide2.png',
      },
      third: {
        img: process.env.PUBLIC_URL + '/assets/slide3.png',
      },
      fourth: {
        img: process.env.PUBLIC_URL + '/assets/slide4.png',
      },
      fifth: {
        img: process.env.PUBLIC_URL + '/assets/slide5.png',
      },
      sixth: {
        img: process.env.PUBLIC_URL + '/assets/slide6.png',
      },
    },
  },
};
